...
| [contracts/civitas-bridge.md](docs/contracts/civitas-bridge.md) | **Sixth and final contract.** `CivitasBridge` — construction-time wiring only, "requests, never reaches in" toward Civitas, `NullPresidiumClient` for opt-in ungoverned mode. `CivitasRuntime` reconciled against `python-civitas`'s real `Runtime.spawn`/`StateStore` source, not left provisional. |
