...
| 1 | **gVisor** — user-space kernel, ~100ms — not implemented; `srt` (below) may cover this platform instead, since it also supports Linux via bubblewrap+netns, not yet exercised here | **`SrtSandbox` — implemented** (`contracts/sandbox.md`), wraps `srt` (Anthropic's Sandbox Runtime, `sandbox-exec`/Seatbelt) — real enforcement confirmed (write/read/network denial, allow-only network via `network.allowedDomains`), live-verified end to end including the guest-shim ZMQ callback bridge, **p50 152ms** ([SPIKE-macos-isolation-srt-libkrun.md](https://github.com/civitas-io/fabrica/blob/main/specs/archive/spikes/SPIKE-macos-isolation-srt-libkrun.md)) | `srt` claims Windows support (`windows-install`: dedicated `srt-sandbox` user + WFP filters) via the same `SrtSandbox` implementation — **untested, deferred**. Windows is a small segment; if `srt` works there, no further action needed. If a real gap surfaces, spike then. |
| 2 | **Firecracker** — microVM, own kernel (KVM). **Implemented** (`FirecrackerSandbox`, `contracts/sandbox.md`) -- real, cold-boot-only v1, validated end to end on real hardware including a real `vsock` tool-call round trip ([SPIKE-firecracker-vsock-callback-bridge.md](https://github.com/civitas-io/fabrica/blob/main/specs/archive/spikes/SPIKE-firecracker-vsock-callback-bridge.md)). Boot: **VMM-ready ~10.5ms**, **full-userspace-ready ~1,055ms with an unoptimized image** — these are different signals, not one number ([SPIKE-firecracker-boot-restore-latency.md](https://github.com/civitas-io/fabrica/blob/main/specs/archive/spikes/SPIKE-firecracker-boot-restore-latency.md)). **Restore from snapshot: 8.1–10.7ms**, validated separately, now REAL and shipped combined with the vsock bridge too (`use_snapshot_restore=True`, opt-in, default `False` preserves cold-boot-only v1 exactly): a real spike (`SPIKE-firecracker-snapshot-restore-vsock-combination.md`) found and fixed two real gaps (a stale vsock socket file `SIGKILL` never cleans up; the guest needing real reconnect logic for a correct `ConnectionResetError` on resume) and a real Firecracker mechanism (`vsock_override`) confirmed working for concurrent restored instances, each getting its own vsock path while sharing one golden rootfs file -- verified safe for this project's own always-terminate-once usage pattern via a real concurrent-write test, not assumed. `PLAN.md` item 20a. **Network isolation: no interface attached at all** (not a filtered one -- no `/network-interfaces` API call is ever made), verified on real hardware: a raw socket connect fails immediately with `ENETUNREACH`, DNS resolution fails immediately too -- structurally stronger than a policy-based allowlist, since there is no interface to misconfigure. | **libkrun** (`Virtualization.framework`-based) — **cold-boot-only, permanently: no snapshot/restore support exists.** This is a structural ceiling, not a bug to fix. Accepted and shippable — snapshot/restore is valuable, not mandatory. | Hyper-V isolation / Windows Sandbox — real, hypervisor-backed, but seconds-to-minutes boot, not milliseconds. Not a near-term priority. |
...
5. **Apple's Containerization framework vs. libkrun for macOS Tier 2** — named as an
   ...
   [SPIKE-macos-isolation-srt-libkrun.md](https://github.com/civitas-io/fabrica/blob/main/specs/archive/spikes/SPIKE-macos-isolation-srt-libkrun.md))
...
6. `srt`'s `--control-fd` persistent-mode — could improve the measured 152ms macOS
   ...
   [SPIKE-prx-invocation-latency.md](https://github.com/civitas-io/fabrica/blob/main/specs/archive/spikes/SPIKE-prx-invocation-latency.md)).
