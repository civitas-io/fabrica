...
actually designed, not just asserted: [mcp-server.md](mcp-server.md)). The
opposite direction — Fabrica as an MCP **client**, consuming an external MCP
server as a `ToolNamespace` — is a separate, complementary design:
